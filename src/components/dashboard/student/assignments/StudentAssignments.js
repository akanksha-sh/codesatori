...
export class StudentAssignments extends Component {
  ...
  componentDidMount() {
    this.setState({
      ongoing: [
        ...DummyTutorialValues.tutorials.filter((tutorial) => tutorial.ongoing),
      ],
      completed: [
        ...DummyTutorialValues.tutorials.filter(
          (tutorial) => !tutorial.ongoing
        ),
      ],
    });
  }
    ...
    return (
      <div style={{ display: "flex", flexDirection: "row", width: "100%" }}>
        ...
        <div style={CompletedTutorialsStyle}>
          <h3>Completed</h3>
          <CompletedTutorials
            tutorials={this.state.completed}
            infoBoxStyle={InfoBoxStyle}
          />
        </div>
      </div>
